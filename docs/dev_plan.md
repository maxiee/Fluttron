...
**当前状态：Phase A 待开始，当前入口版本为 v0075**
...
|---|---|---|---|---|---|
| v0075 | 修复 `fluttron_cli` 全部 13 个 `avoid_single_cascade_in_expression_statements` lint warnings（位于 `ui_build_pipeline_test.dart`）；确认 `analysis_options.yaml` 配置正确 | `docs/feature/v1_release_roadmap.md §v0075` | 无 | `dart analyze packages/fluttron_cli` 与 `dart analyze packages/fluttron_shared` 均报告 0 issues；现有 400+ 测试全部通过 | ⬜ 待开始 |
| v0076 | 统一所有包 `pubspec.yaml` 版本为 `0.1.0-dev`；为所有包补全有意义的 `description` 字段（无 "A new Flutter project"）；为 `fluttron_cli`/`fluttron_shared` 设置 `repository` 字段 | `docs/feature/v1_release_roadmap.md §v0076` | v0075 | 5 个 pubspec 描述有意义；版本均为 `0.1.0-dev`；`dart pub get` 全部成功；无新 analyze 问题 | ⬜ 待开始 |
| v0077 | 创建 `.github/workflows/ci.yml`，包含 `test-cli`（`dart test --exclude-tags acceptance`）和 `test-shared`（`dart test`）两个 job | `docs/feature/v1_release_roadmap.md §v0077` | v0076 | `.github/workflows/ci.yml` 存在；本地 `dart test --exclude-tags acceptance`（CLI）通过；本地 `dart test`（shared）通过 | ⬜ 待开始 |
...
- **当前入口**：`v0075`（修复 `fluttron_cli` lint warnings，建立零 issue 代码基线）
- **技术方案参阅**：`docs/feature/v1_release_roadmap.md §v0075`（`### v0075 — Fix all lint warnings and standardize analysis options`）
- **最小验收**：`dart analyze packages/fluttron_cli` 与 `dart analyze packages/fluttron_shared` 均报告 0 issues；`dart test --exclude-tags acceptance`（CLI）全通过
- 已完成的重大能力：

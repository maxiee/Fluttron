...
- 新增 `packages/fluttron_cli/lib/src/utils/registration_generator.dart`：
  - 定义 `RegistrationGenerator` 类，实现 `generate()` 和 `generateSync()` 方法
  - 定义 `RegistrationResult` 模型类（outputPath、packageCount、factoryCount、hasGenerated）
  - 定义 `RegistrationGeneratorException` 异常类
- 核心功能：
  - 生成 `ui/lib/generated/web_package_registrations.dart` 文件
  - 输出 `registerFluttronWebPackages()` 函数，注册所有视图工厂
  - 支持多包多工厂注册，按包名分组添加注释
  - 自动创建输出目录，空包时生成占位文件
  - 单引号转义处理，确保生成的 Dart 代码语法正确
  - `@generated` 头注释与 `ignore_for_file: directives_ordering`
- 生成代码格式：
  ```dart
  ...
  ```
- 新增测试 `packages/fluttron_cli/test/src/utils/registration_generator_test.dart`：
  - 14 个测试覆盖：模型类、单包单工厂、单包多工厂、多包、空包、目录创建、自定义输出路径、转义、同步/异步一致性、常量校验
- 验收通过：`dart test`（`packages/fluttron_cli`）138 个测试全部通过

...
**当前状态：Phase A 已全部完成（v0075-v0079），v0080-v0101 已完成，Phase D 已全部完成，当前入口版本为 v0102**
...
| v0087 | 为 `PackageCommand` 增加 `--dmg` flag，调用 `hdiutil create` 生成 .dmg 文件 | `docs/feature/v1_release_roadmap.md §v0087` | v0086 | `fluttron package -p playground --dmg` 生成 .dmg；DMG 可挂载并包含 app；大小打印至 stdout | ✅ 已完成 |
| v0088 | 更新 `examples/markdown_editor`：使用 `WindowServiceClient` 在打开文件时动态设置窗口标题（`Fluttron Editor - filename.md`）；使用 `LoggingServiceClient` 记录关键操作日志 | `docs/feature/v1_release_roadmap.md §v0088` | v0083, v0084 | 打开文件时窗口标题动态变化；host 控制台可见操作日志；原有 markdown_editor 功能无回归 | ✅ 已完成 |
...
| v0092 | 创建 `packages/fluttron_cli/lib/src/version.dart`（`const fluttronVersion = '0.1.0-dev'`）；在 CLI `CommandRunner` 中配置 version；验证 `fluttron --version` 输出正确 | `docs/feature/v1_release_roadmap.md §v0092` | v0091 | `fluttron --version` 输出 `0.1.0-dev`；所有 pubspec 版本与 version.dart 一致 | ✅ 已完成 |
| v0093 | 实现 `fluttron doctor` 命令，检查 Flutter SDK / Dart SDK / Node.js / pnpm / macOS 桌面支持已启用；格式化输出 ✓/✗；exit code 0（全通过）或 1（有失败）；注册至 CLI；补测试 | `docs/feature/v1_release_roadmap.md §v0093` | v0092 | `fluttron doctor` 可运行并打印环境状态；缺失依赖明确标注；exit code 符合预期 | ✅ 已完成 |
| v0094 | 更新 `.gitignore` 覆盖 `.opencode/`、`.test_integration/`、`.pnpm-store/`、`.mcp_servers/` 等开发工具目录；整理仓库根目录，确保无内部专用文件暴露给外部访客 | `docs/feature/v1_release_roadmap.md §v0094` | v0093 | `.gitignore` 覆盖所有开发工具目录；仓库根目录对外部访客整洁 | ✅ 已完成 |

#### Phase D: Documentation & Presentation（v0095-v0101）

| 版本 | 最小可执行任务 | 技术方案必读章节 | 依赖 | 最小验收 | 状态 |
|---|---|---|---|---|---|
| v0095 | 重写 `README.md`（英文）：tagline、badges（CI/version/license）、截图/GIF 占位、What is Fluttron / Why Fluttron 对比表 / Quick Start / Features / Architecture Mermaid 图 / Examples / Contributing / License | `docs/feature/v1_release_roadmap.md §v0095` | v0094 | README 有对比表（vs Electron/Tauri/Flutter Desktop）、CI badge、Quick Start；结构完整无占位符 | ✅ 已完成 |
| v0096 | 创建 `README-zh.md`（中文完整翻译）；两个 README 顶部互相添加语言切换链接 | `docs/feature/v1_release_roadmap.md §v0096` | v0095 | README-zh.md 存在；两文件顶部有互链（`[中文](README-zh.md)` / `[English](README.md)`） | ✅ 已完成 |
...
| v0101 | 构建运行 `examples/markdown_editor`，截图保存至 `docs/screenshots/`；录制 CLI create-build-run 流程 GIF（使用 `vhs` 或 `asciinema`）；更新 README.md 引用截图/GIF | `docs/feature/v1_release_roadmap.md §v0101` | v0100 | ≥1 张截图 + ≥1 个 GIF 存在于 `docs/screenshots/`；README.md 展示视觉资产；单文件 <2MB | ✅ 已完成 |
...
- **当前入口**：`v0102`（版本升 0.1.0-alpha + 全量测试扫描）
- **技术方案参阅**：`docs/feature/v1_release_roadmap.md §v0102`
- **最小验收**：所有版本为 `0.1.0-alpha`；所有测试通过；所有 analyze 干净；playground/markdown_editor/host_service_demo 均可构建运行
- 已完成的重大能力：

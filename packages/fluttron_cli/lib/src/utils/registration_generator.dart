...
class RegistrationGenerator {
  ...
  Future<RegistrationResult> generate({
    ...
    // Calculate totals
    final factoryCount = packages.fold<int>(
      0,
      (sum, pkg) => sum + pkg.viewFactories.length,
    );
  ...
  String _generateCodeContent(List<WebPackageManifest> packages) {
    ...
    // Generate registration calls grouped by package
    for (final package in packages) {
      ...
      // Generate registration for each view factory
      for (final factory in package.viewFactories) {
        buffer.writeln('  FluttronWebViewRegistry.register(');
        buffer.writeln('    const FluttronWebViewRegistration(');
        buffer.writeln("      type: '${_escapeString(factory.type)}',");
        buffer.writeln(
          "      jsFactoryName: '${_escapeString(factory.jsFactoryName)}',",
        );
        buffer.writeln('    ),');
        buffer.writeln('  );');
      }
